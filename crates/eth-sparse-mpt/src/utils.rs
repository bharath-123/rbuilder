...
pub fn strip_first_nibble_mut(p: &mut Nibbles) -> u8 {
    let mut new_vec = p.to_vec();
    let first_element = new_vec.remove(0);
    let mut updated_nibble = Nibbles::new();
    updated_nibble.extend_from_slice(new_vec.as_slice());
    *p = updated_nibble;
    first_element
}
...
pub fn extract_prefix_and_suffix(p1: &Nibbles, p2: &Nibbles) -> (Nibbles, Nibbles, Nibbles) {
    ...
    let prefix = p1.slice(..prefix_len);
    let suffix1 = p1.slice(prefix_len..);
    let suffix2 = p2.slice(prefix_len..);
